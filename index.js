...
function makePersonObject(id, name, email) {
  const person = {id:id, name:name, email:email}
  return person
}
...
function makeSmartPerson(name) {
  const obj = {name:name, sum: function(n1, n2){
    return n1 + n2
  }, speak: function(){
    return `Hello, my name is ${obj.name}`
  }}
...
function sortCarInventory(inventory) {
  const inventorySortedByCarModel = inventory.sort((a, b) =>
...
function getGermanCars(arr) {
  return arr.filter(element => {
    return element.car_make === 'Audi' || element.car_make === 'Mercedes-Benz' || element.car_make === 'Volkswagen' || element.car_make === 'BMW'
  })
}
...
function carMaker(meter) {
  const obj = {odometer:meter, drive: function(distance){
    return obj.odometer = obj.odometer + distance
  }}
  return obj
}
